...
class Instance: Codable {
    ...
    static func register(name: String, serverUrl: String) async throws -> (Instance, DecentralisClient) {
        let client = try await DecentralisClient(serverUrl: serverUrl)
    ...
    func open() async throws -> DecentralisClient {
        if let accessToken = self.accessToken {
            return try DecentralisClient(
            ...
            return try await DecentralisClient(
                serverUrl: self.serverUrl,
                userId: self.userId,
                privateKeyEncoded: self.privateKey
            )
